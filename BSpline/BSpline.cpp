...
template<class T> BSplinePlus<T>::BSplinePlus(const T *x,
    ...
    for (int i = 1; i < nx; i++) {
        ...
        assert(x[i-1] < x[i]);
        // determine the x indices of the dat poiunts that lie outside of the 
        // blending range. 
        if (_blendMode == BSplinePlus<T>::BLENDSTART || _blendMode
                == BSplinePlus<T>::BLENDBOTH) {
            if (x[i-1] < _blendSpan)
                _blendXleftIndex = i+1;
        }
    ...
    std::cout << "blend span is on either end of indices " << _blendXleftIndex
            << ":" << _blendXrightIndex << "\n";
