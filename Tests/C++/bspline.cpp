...
void DumpSpline(vector<datum>& x, vector<datum>& y, SplineT& spline,
                ostream* out, bool debug);

#if OOYAMA
static bool vic(float* xt, int nxp, double wl, int bc, float* y);
extern "C"
{
    void vicsetup_(float* xt, int* nxp, float* ydcwl, int* nx, float* ynb,
                   float* ynt, float* fmin, int* ierr, int* echo);
    void vicspl_(float* xt, float* xd, float* y, int* nxp, int* nxp, int* kdat,
                 float* ynb, float* ynt, int* nx, float* ydcwl, int* kybc,
                 int* kybc, float* ydcwl, float* ydcwl, int* ierr);
                 ...
                 int& num_nodes, bool& debug)
{
    ...
    while ((optchar = opts(iter, optarg)))
    {
        ...
        case 'v':
        {
            cout << "BSpline version: " << SplineBase::Version() << endl;
...
main(int argc, char* argv[])
{
    ...
    parseCommandLine(argc, argv, infile, outfile, step, wavelength, bc,
                     num_nodes, debug);
    ...
    if (debug)
        SplineT::Debug(1);
    SplineT spline(&x[0], x.size(), &y[0], wavelength, bc, num_nodes);
    if (spline.ok())
    {
...
void
DumpSpline(vector<datum>& xv, vector<datum>& yv, SplineT& spline, ostream* out,
           bool debug)
{
    // write column headings
    *out << setw(10) << "x" << setw(10) << "y" << setw(15) << "spline(x)"
         << setw(20) << "slope(spline(x))" << std::endl;
...
vic(float* xt, int nxp, double wl, int bc, float* y)
{
    ...
    ierr = 1;
    vicspl_(xt, xw.begin(), /*xdat*/ y, &nxp, &nxp, &kdat, &ynb, &ynt, &nx,
