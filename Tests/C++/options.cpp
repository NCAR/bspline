...
const unsigned  OptIstreamIter::MAX_LINE_LEN = 1024 ;
   ...
   // Constructor
OptIstreamIter::OptIstreamIter(std::istream & input) : is(input), tok_iter(NULL)
{
#ifdef  USE_STDIO
...
OptionSpec::isSyntaxError(const char * name) const {
   int  error = 0;
   if ((! spec) || (! *spec)) {
	   std::cerr << name << ": empty option specifier." << std::endl;
      std::cerr << "\tmust be at least 1 character long." << std::endl;
      ++error;
   } else if (spec[1] && (strchr("|?:*+", spec[1]) == NULL)) {
      std::cerr << name << ": bad option specifier \"" << spec << "\"." << std::endl;
      std::cerr << "\t2nd character must be in the set \"|?:*+\"." << std::endl;
      ++error;
   }
   return  error;
}
...
Options::parse_opt(OptIter & iter, const char * & optarg) {
   ...
   if (optspec.isNULL()) {
      ...
      if (! (optctrls & Options::QUIET)) {
         std::cerr << cmdname << ": unknown option -"
              << *(nextchar - 1) << "." << std::endl ;
      }
   ...
   if (optspec.isValRequired() &&  !(optctrls & Options::QUIET)) {
      std::cerr << cmdname << ": argument required for -" << optspec.OptChar()
           << " option." << std::endl ;
   }
   return  optspec.OptChar();
}
...
int Options::parse_longopt(OptIter & iter, const char * & optarg) {
   ...
   if (optspec.isNULL()) {
      ...
      if (! (optctrls & Options::QUIET)) {
         std::cerr << cmdname << ": " << ((ambiguous) ? "ambiguous" : "unknown")
              << " option "
   ...
   if (optspec.isNoArg()) {
      if ((val) && ! (optctrls & Options::QUIET)) {
         std::cerr << cmdname << ": option "
              << ((optctrls & Options::LONG_ONLY) ? "-" : "--")
              << optspec.LongOpt() << " does NOT take an argument." << std::endl ;
      }
   ...
   if (optspec.isValRequired() &&  !(optctrls & Options::QUIET)) {
      ...
      std::cerr << cmdname << ": argument required for "
           << ((optctrls & Options::LONG_ONLY) ? "-" : "--");
      std::cerr.write(longopt, longopt_len) << " option." << std::endl ;
   }
   nextchar = NULLSTR;           // we exhausted the rest of this arg
   return  optspec.OptChar();
}
...
// ^^-------------------------------------------------------------------------
void
Options::usage(std::ostream & os, const char * positionals) const {
#ifdef NO_USAGE
   return;
#else
