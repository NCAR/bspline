...
public:
    ...
    // The returned object will need to be deleted.
    BSpline *apply (const float *y);

    // These methods return info about the spline domain.  The array of
    // nodes returned by nodes() belongs to the object and should not be
    // deleted; it will also be invalid if the object is destroyed.
    ...
    // Return the Alpha value for a given wavelength.  With no argument,
    // return the object's current alpha.
